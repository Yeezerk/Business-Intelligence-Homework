"""
训练 FTT + Softmax + Focal Loss（手写实现，ReGLU，匹配官方架构）。
"""
...
def main():
    ...
    print(f"FTT-Softmax (ReGLU) params: {n_params:,}")
    ...
    trainer = Trainer(model=model, loss_fn=loss_fn, optimizer=optimizer,
                      scheduler=scheduler, model_name="FTT-Softmax-ReGLU")
    ...
    torch.save(model.state_dict(), os.path.join(MODEL_DIR, "ftt_softmax_best.pt"))
    print(f"\nFTT-Softmax Results (threshold={best_threshold:.3f}):")
    for k, v in metrics.items():
        print(f"  {k}: {v}")

    import pandas as pd
    pd.DataFrame([metrics]).to_csv(os.path.join(RESULT_DIR, "ftt_softmax_metrics.csv"), index=False)
