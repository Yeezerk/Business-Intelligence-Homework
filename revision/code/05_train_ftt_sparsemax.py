"""
训练 FTT + Sparsemax + Focal Loss（手写实现，ReGLU）。
与 04_train_ftt_softmax.py 唯一变量：use_sparsemax=True vs False。
"""
...
def main():
    ...
    reset_seed(SEED)
    model = FTTransformer(input_dim, use_sparsemax=True).to(DEVICE)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"FTT-Sparsemax (ReGLU) params: {n_params:,}")
    ...
    trainer = Trainer(model=model, loss_fn=loss_fn, optimizer=optimizer,
                      scheduler=scheduler, model_name="FTT-Sparsemax-ReGLU")
    ...
    torch.save(model.state_dict(), os.path.join(MODEL_DIR, "ftt_sparsemax_best.pt"))
    print(f"\nFTT-Sparsemax Results (threshold={best_threshold:.3f}):")
    for k, v in metrics.items():
        print(f"  {k}: {v}")

    import pandas as pd
    pd.DataFrame([metrics]).to_csv(os.path.join(RESULT_DIR, "ftt_sparsemax_metrics.csv"), index=False)
